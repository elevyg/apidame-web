...
const EsteticaHero = forwardRef<HTMLElement>(function EsteticaHero(_, ref) {
      ...
      <nav className="absolute top-5 right-5 z-10 flex flex-col items-end md:top-8 md:right-16 lg:right-24">
        {esteticaLinks.map((item) => (
          <a
            key={item.href}
            href={item.href}
            className="border-r border-paper/40 py-2.5 pr-3 font-brown text-[11px] tracking-[0.2em] text-paper/85 uppercase [text-shadow:0_1px_16px_rgba(0,0,0,0.45)] hover:text-paper md:text-xs"
          >
            {item.label}
          </a>
        ))}
